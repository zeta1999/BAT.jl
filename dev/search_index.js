...

{
    ...
    "category": "section",
    "text": "Welcome to BAT, the Bayesian analysis toolkit. This is a (still incomplete) rewrite of the previous C++ version in Julia."
},

{
    ...
    "title": "Installation",
    "category": "section",
    "text": "BAT.jl is under development and not a registered Julia package yet. Install viausing Pkg\npkg\"add https://github.com/BAT/BAT.jl.git\""
},

{
    ...
    "category": "section",
    "text": "When changing the code of BAT.jl and testing snippets and examples in the REPL, automatic code reloading comes in very handy. Try out Revise.jl."
},
...
{
    "location": "tutorial/#Starting-with-a-Normal-Distribution-1",
    "page": "Tutorial",
    "title": "Starting with a Normal Distribution",
    "category": "section",
    "text": "First we need to set up the environment:julia> using BATFor our example we need additional packages:julia> using Distributions, PDMats, StatsBaseNow we can define a Multi-variate Normal Distribution:julia> mvec = [-0.3, 0.3]\njulia> Σ = PDMat([1.0 1.5; 1.5 4.0])\njulia> density = MvDistDensity(MvNormal(mvec, Σ))Here we will use the MetroplisHastings-algorithm:julia> algorithm = MetropolisHastings()We want our samples to be in the following bounds with reflective property:julia> bounds = HyperRectBounds([-5, -8], [5, 8], reflective_bounds)To separate general MCMC specifications from specific MCMC settings and to hold this information collectively the MCMCSpec type was introduced:julia> spec = MCMCSpec(algorithm, density, bounds)At last we define the total number of Markov-Chains and how many samples we want for every chain:julia> nsamples_per_chain = 2000\njulia> nchains = 4Then, we draw the samples with rand:julia> samples, sampleids, stats = rand(\n                        spec,\n                        nsamples_per_chain,\n                        nchains,\n                        max_time = Inf,\n                        granularity = 1\n                        )               We can check e.g. the sample covariance matrix:julia> cov(samples.params, FrequencyWeights(samples.weight), 2; corrected=true)\n2×2 Array{Float64,2}:\n 0.971406  1.45419\n 1.45419   3.97268Indeed, the estimates are close to the true values of Σ = [1.0 1.5; 1.5 4.0] which we used to specify the Normal Distribution."
},
...

{
    ...
    "category": "section",
    "text": "A test equationfracnk(n - k) = binomnk"
},
...
{
    "location": "api/#BAT.gr_Rsqr-Tuple{AbstractArray{#s117,1} where #s117<:MCMCBasicStats}",
    "page": "API",
    "title": "BAT.gr_Rsqr",
    "category": "method",
